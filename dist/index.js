...
function assertServerName(value, field = "server") {
    const serverName = assertNoControlChars(assertString(value, field), field);
    if (!SAFE_SERVER_RE.test(serverName)) {
        throw new Error(`${field} must match ${SAFE_SERVER_RE.toString()} (letters, numbers, ., _, -)`);
    }
    return serverName;
}
function assertProjectName(value) {
    ...
    if (!SAFE_PROJECT_RE.test(projectName)) {
        throw new Error(`project_name must match ${SAFE_PROJECT_RE.toString()} (letters, numbers, ., _, -)`);
    }
    return projectName;
}
...
function assertTimeoutSeconds(value, field = "timeout") {
...
function assertSyncMethod(value) {
...
function assertDevSessionMode(value) {
...
function assertStringArray(value, field) {
    ...
    return value.map((item, idx) => assertNoControlChars(assertString(item, `${field}[${idx}]`), `${field}[${idx}]`));
}
function shellQuote(value) {
    return `'${value.replace(/'/g, `'\"'\"'`)}'`;
}
function shellQuoteArgs(parts) {
    return parts.map((part) => shellQuote(part)).join(" ");
}
async function executeCommand(program, args, timeoutMs = 300000, cwd) {
...
function buildSSHBaseArgs(server, config) {
...
function buildSSHArgs(server, config, remoteCommand) {
    const { args, target } = buildSSHBaseArgs(server, config);
    return [...args, target, remoteCommand];
}
function buildSCPArgs(server, config, direction, localPath, remotePath) {
    const { args, target } = buildSSHBaseArgs(server, config);
    const scpArgs = ["-r"];
...
function resolveScpUploadLocalSources(localPath) {
    try {
        const stats = statSync(localPath);
...
function resolveRsyncSourcePath(localPath) {
    try {
        const stats = statSync(localPath);
        if (stats.isDirectory()) {
...
async function executeRemoteCommand(server, config, remoteCommand, timeoutMs = 300000) {
    const sshArgs = buildSSHArgs(server, config, remoteCommand);
    return executeCommand("ssh", sshArgs, timeoutMs);
}
function parseBackgroundPid(rawOutput) {
...
function isNothingToCommit(text) {
    const normalized = text.toLowerCase();
    return normalized.includes("nothing to commit") || normalized.includes("nothing added to commit");
}
function isRsyncUnavailable(result) {
    const combined = `${result.stdout}\n${result.stderr}`.toLowerCase();
    if (result.exitCode === 127)
        return true;
    return (combined.includes("rsync: command not found") ||
...
async function performRemoteSync(params) {
    ...
    const safeDeleteRemoteExtra = typeof params.delete_remote_extra === "boolean" ? params.delete_remote_extra : false;
    ...
    if (safeMethod !== "scp") {
        ...
        rsyncArgs.push(rsyncSourcePath, buildRsyncRemoteTarget(safeServer, config, safeRemotePath));
        ...
        if (rsyncResult.exitCode === 0) {
            ...
            await executeRemoteCommand(safeServer, config, `mkdir -p ${shellQuote(remoteMkdirPath)}`, 30000);
            if (scpSourcePaths.length === 0) {
                result = { stdout: "", stderr: "", exitCode: 0 };
            }
            else {
                const scpArgs = buildSCPArgs(safeServer, config, "upload", scpSourcePaths, safeRemotePath);
    ...
    return {
        ...
        rsync_error: fallbackUsed && rsyncResult
...
function assertSessionName(value) {
...
function shouldIgnorePath(relativePath, excludePatterns) {
    ...
    for (const pattern of excludePatterns) {
        ...
        if (!p.includes("*")) {
            if (p.includes("/")) {
                ...
                if (normalized === p || normalized.includes(`/${p}/`) || base === p)
                    return true;
            }
            continue;
        }
...
function markSessionError(session, error) {
...
async function maybeStartLongTask(session) {
    if (session.mode !== "long" || !session.longCommand)
        return;
    if (session.pid) {
        const checkRes = await executeRemoteCommand(session.server, config, `ps -p ${session.pid} -o pid=,stat=,etime=,command=`, 15000);
    ...
    const actualLogFile = session.logFile || `${session.remoteWorkingDir}/${session.name}_${timestamp}.log`;
...
async function pollLongTask(session) {
...
async function runDevSessionCycle(session, reason) {
...
function scheduleDevSessionCycle(session, reason) {
    if (session.stopRequested)
        return;
    if (session.debounceTimer) {
        clearTimeout(session.debounceTimer);
...
function startDevSessionWatcher(session) {
    const watcher = watch(session.localPath, { recursive: true }, (_eventType, filename) => {
        if (session.stopRequested)
            return;
        if (!filename) {
        ...
        if (shouldIgnorePath(changed, session.exclude))
            return;
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            {
                name: "remote_execute",
                description: "【短任务】在远程服务器上执行命令，同步等待完成并返回结果。适用于：ls、cat、pip install、简单脚本（< 5分钟）。超过 5 分钟的任务请用 remote_run_background。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            // ========== 后台任务（长时间运行） ==========
            {
                ...
                description: "【长任务】用 nohup 在后台启动任务，立即返回 PID 和日志路径。适用于：模型训练、数据处理、长时间脚本（> 5分钟）。启动后用 remote_tail_log 查看日志，remote_check_process 检查状态。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            },
            {
                ...
                description: "【长任务配套】查看后台任务的日志文件。可显示最后 N 行，或实时跟踪指定秒数。配合 remote_run_background 使用。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            },
            {
                ...
                description: "【长任务配套】检查后台进程是否还在运行。通过 PID 或进程名查询。配合 remote_run_background 使用。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            {
                name: "remote_sync",
                description: "将本地文件/目录同步到远程服务器。使用 rsync 或 scp。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            // ========== Git 操作（双向） ==========
            {
                ...
                description: "【本地→服务器】在指定本地目录执行 git add、commit、push 到服务器。代码会自动部署到服务器工作目录。必须指定 working_dir。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            },
            {
                ...
                description: "【服务器→本地】首次从服务器 clone 仓库到本地。用于获取服务器上已有的代码。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            },
            {
                ...
                description: "【服务器→本地】在指定本地目录执行 git pull，从服务器拉取最新代码。必须指定 working_dir。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            },
            {
                ...
                description: "【初始化】在服务器上创建 Git 仓库（bare repo + 工作目录 + 自动部署 hook）。新项目首次使用。",
                inputSchema: {
                    type: "object",
                    properties: {
                        server: {
            ...
            // ========== 固定化开发会话 ==========
            {
                ...
                description: "启动固定化开发会话：监听本地文件变化，自动同步到远程，并按模式执行（short 立即运行；long 后台运行并轮询日志）。",
                inputSchema: {
                    type: "object",
                    properties: {
                        session_name: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            // ========== 基础执行 ==========
            case "remote_execute": {
                const { server: serverName, command, working_dir, timeout = 300 } = args;
            ...
            case "remote_run_background": {
                const { server: serverName, command, working_dir, log_file, task_name = "task", } = args;
                const safeServer = assertServerName(serverName);
                const safeCommand = assertString(command, "command");
                const safeWorkingDir = assertPath(working_dir, "working_dir");
            ...
            case "remote_tail_log": {
                const { server: serverName, log_file, lines = 50, follow_seconds = 0, } = args;
                const safeServer = assertServerName(serverName);
            ...
            case "remote_check_process": {
                const { server: serverName, pid, process_name, working_dir, } = args;
                const safeServer = assertServerName(serverName);
                let checkCmd;
                let isRunning;
                if (pid) {
                    ...
                    const safeProcessName = assertNoControlChars(assertString(process_name, "process_name"), "process_name");
                    if (working_dir) {
                        const safeWorkingDir = assertPath(working_dir, "working_dir");
            ...
            case "remote_kill_process": {
                const { server: serverName, pid, signal = "TERM", } = args;
                const safeServer = assertServerName(serverName);
                const safePid = assertPositiveInt(pid, "pid");
            ...
            // ========== 文件同步 ==========
            case "remote_sync": {
                const { server: serverName, local_path, remote_path, exclude = [], method = "auto", delete_remote_extra = false, } = args;
            ...
            case "remote_pull": {
                const { server: serverName, remote_path, local_path } = args;
                const safeServer = assertServerName(serverName);
            ...
            case "remote_git_push": {
                const { server: serverName, commit_message, working_dir, branch = "main", } = args;
                const safeServer = assertServerName(serverName);
                const safeCommitMessage = assertNoControlChars(assertString(commit_message, "commit_message"), "commit_message");
                const safeWorkingDir = assertPath(working_dir, "working_dir");
                ...
                const commitResult = await executeCommand("git", ["commit", "-m", safeCommitMessage], 60000, safeWorkingDir);
                ...
                const pushResult = await executeCommand("git", ["push", "-u", safeServer, safeBranch], 60000, safeWorkingDir);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "remote_git_clone": {
                const { server: serverName, remote_repo_path, local_path, branch = "main", } = args;
                const safeServer = assertServerName(serverName);
                const safeRemoteRepoPath = assertPath(remote_repo_path, "remote_repo_path");
                const safeLocalPath = assertPath(local_path, "local_path");
                const safeBranch = assertBranch(branch);
                const cloneUrl = `${safeServer}:${safeRemoteRepoPath}`;
                const result = await executeCommand("git", ["clone", "-b", safeBranch, cloneUrl, safeLocalPath], 120000);
                // 如果 clone 成功，添加远程
                if (result.exitCode === 0) {
                    await executeCommand("git", ["remote", "rename", "origin", safeServer], 10000, safeLocalPath);
                }
                return {
                    content: [
                        {
            ...
            case "remote_git_pull_local": {
                const { server: serverName, working_dir, branch = "main", } = args;
                const safeServer = assertServerName(serverName);
                const safeWorkingDir = assertPath(working_dir, "working_dir");
                const safeBranch = assertBranch(branch);
                const result = await executeCommand("git", ["pull", safeServer, safeBranch], 60000, safeWorkingDir);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "remote_git_init": {
                const { server: serverName, project_name, bare_repo_path, work_dir_path, } = args;
                const safeServer = assertServerName(serverName);
...
echo "deployed to ${actualWorkDir}"`;
            ...
            case "dev_session_start": {
                const { session_name = "dev-session", server: serverName, local_path, remote_path, remote_working_dir, mode = "short", short_command, long_command, exclude = [], method = "auto", delete_remote_extra = false, debounce_ms = 800, poll_interval_seconds = 5, log_lines = 80, follow_seconds = 1, } = args;
                ...
                const safeMethod = assertSyncMethod(method);
                const safeDeleteRemoteExtra = typeof delete_remote_extra === "boolean" ? delete_remote_extra : false;
                const safeDebounceMs = assertPositiveInt(debounce_ms, "debounce_ms");
                const safePollInterval = assertPositiveInt(poll_interval_seconds, "poll_interval_seconds");
            ...
            case "dev_session_status": {
                const { session_id, trigger_now = false } = args;
            ...
            case "dev_session_stop": {
                const { session_id, stop_remote_process = false, signal = "TERM" } = args;
                const safeSessionId = assertNoControlChars(assertString(session_id, "session_id"), "session_id");
                const session = devSessions.get(safeSessionId);
                if (!session) {
                    throw new Error(`session not found: ${safeSessionId}`);
                ...
                if (stop_remote_process && session.mode === "long" && session.pid) {
                    ...
                    const killRes = await executeRemoteCommand(session.server, config, `kill -s ${safeSignal} ${session.pid}`, 10000);
            ...
            case "test_connection": {
                ...
                const safeServer = assertServerName(serverName);
                const result = await executeRemoteCommand(safeServer, config, `echo "Connection successful" && hostname && uname -a`, 10000);
                return {
                    content: [
                        {
                            type: "text",
